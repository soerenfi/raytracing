...
/* changes by soeren.fi@icloud.com
...
void AppBase::create(const AppBaseVkCreateInfo& info) {
  m_useDynamicRendering = info.useDynamicRendering;
  setup(info.instance, info.device, info.physicalDevice, info.queueIndices[0]);
  createSwapchain(info.surface, info.size.width, info.size.height, VK_FORMAT_B8G8R8A8_UNORM, VK_FORMAT_UNDEFINED,
                  info.useVsync);
  createDepthBuffer();
  createRenderPass();
  createFrameBuffers();
...
//
void AppBase::setup(const VkInstance& instance, const VkDevice& device, const VkPhysicalDevice& physicalDevice,
...
//
VkSurfaceKHR AppBase::getVkSurface(const VkInstance& instance, GLFWwindow* window) {
  assert(instance);
  m_window = window;
...
//
void AppBase::createSwapchain(const VkSurfaceKHR& surface, uint32_t width, uint32_t height,
                              VkFormat colorFormat /*= VK_FORMAT_B8G8R8A8_UNORM*/,
                              VkFormat depthFormat /*= VK_FORMAT_UNDEFINED*/, bool vsync /*= false*/) {
...
void AppBase::createRenderPass() {
  ...
  // Transition from final to initial (VK_SUBPASS_EXTERNAL refers to all commands executed outside of the actual
  // renderpass)
...
void AppBase::createDepthBuffer() {
  ...
  vkCmdPipelineBarrier(cmdBuffer, srcStageMask, destStageMask, VK_FALSE, 0, nullptr, 0, nullptr, 1,
                       &imageMemoryBarrier);
  submitTempCmdBuffer(cmdBuffer);
...
void AppBase::prepareFrame() {
  ...
  if (result != VK_SUCCESS) {  // This allows Aftermath to do things and later assert below
#ifdef _WIN32
    Sleep(1000);
#else
    usleep(1000);
...
void AppBase::submitFrame() {
  ...
  submitInfo.pWaitSemaphores =
    &semaphoreRead;  // Semaphore(s) to wait upon before the submitted command buffer starts executing
  ...
  submitInfo.pCommandBuffers =
    &m_commandBuffers[imageIndex];    // Command buffers(s) to execute in this batch (submission)
...
//
void AppBase::setViewport(const VkCommandBuffer& cmdBuf) {
...
void AppBase::updateCamera() {
  ...
  if (!(ImGui::IsKeyDown(ImGuiKey_ModAlt) || ImGui::IsKeyDown(ImGuiKey_ModCtrl) ||
...
//
void AppBase::initGUI(uint32_t subpassID /*= 0*/) {
...
//
void AppBase::fitCamera(const nvmath::vec3f& boxMin, const nvmath::vec3f& boxMax, bool instantFit /*= true*/) {
...
//
bool AppBase::isMinimized(bool doSleeping /*= true*/) {
  int w, h;
...
void AppBase::framebuffersize_cb(GLFWwindow* window, int w, int h) {
  auto app = static_cast<AppBase*>(glfwGetWindowUserPointer(window));
  app->onFramebufferSize(w, h);
}

void AppBase::mousebutton_cb(GLFWwindow* window, int button, int action, int mods) {
  auto app = static_cast<AppBase*>(glfwGetWindowUserPointer(window));
  app->onMouseButton(button, action, mods);
}

void AppBase::cursorpos_cb(GLFWwindow* window, double x, double y) {
  auto app = static_cast<AppBase*>(glfwGetWindowUserPointer(window));
  app->onMouseMotion(static_cast<int>(x), static_cast<int>(y));
}

void AppBase::scroll_cb(GLFWwindow* window, double x, double y) {
  auto app = static_cast<AppBase*>(glfwGetWindowUserPointer(window));
  app->onMouseWheel(static_cast<int>(y));
}

void AppBase::key_cb(GLFWwindow* window, int key, int scancode, int action, int mods) {
  auto app = static_cast<AppBase*>(glfwGetWindowUserPointer(window));
  app->onKeyboard(key, scancode, action, mods);
}

void AppBase::char_cb(GLFWwindow* window, unsigned int key) {
  auto app = static_cast<AppBase*>(glfwGetWindowUserPointer(window));
  app->onKeyboardChar(key);
}

void AppBase::drop_cb(GLFWwindow* window, int count, const char** paths) {
  auto app = static_cast<AppBase*>(glfwGetWindowUserPointer(window));
  int  i;
  for (i = 0; i < count; i++) app->onFileDrop(paths[i]);
}

uint32_t AppBase::getMemoryType(uint32_t typeBits, const VkMemoryPropertyFlags& properties) const {
...
VkCommandBuffer AppBase::createTempCmdBuffer() {
...
void AppBase::submitTempCmdBuffer(VkCommandBuffer cmdBuffer) {
